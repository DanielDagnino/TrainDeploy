...
from is_ai.utils.torch.decorators import preserve_trainability
from is_ai.model.helpers import GEM_ONNX
...
class AudioClf(BaseModel):
    def __init__(self, cfg):
        ...
        #     # self.backbone = get_model(num_classes=1, width_mult=1.0, pretrained_name="mn10_as")
        #     self.backbone = get_mobilenet(width_mult=NAME_TO_WIDTH(self.name), pretrained_name=self.name,
        #                           head_type="mlp", se_dims="c", strides=(2, 2, 2, 2))
        #     self.predictor_dropout = None
        #     self.n_backbone_features = None
        #     self.gem = None
        #     self.fc = None
        #     self.mel = AugmentMelSTFT(n_mels=128, sr=32_000, win_length=800, hopsize=320)

        elif self.name == "ecaresnet50t":
            self.backbone = timm.create_model(cfg.backbone.name, **cfg.backbone.args)
            self.predictor_dropout = None
            self.n_backbone_features = self.backbone.feature_info[-1]['num_chs']
            self.gem = GEM_ONNX(previous_model_layers=self.backbone, img_size=cfg.args.img_size,
                                gem_power=cfg.args.gem_p, gem_train=cfg.args.gem_train, gem_type=cfg.args.gem_type)
            self.fc = nn.Linear(self.n_backbone_features, self.n_classes, bias=True)
